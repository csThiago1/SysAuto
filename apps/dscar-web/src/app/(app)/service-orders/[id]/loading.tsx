export default function ServiceOrderLoading() {
  return (
    <div className="flex h-[calc(100vh-64px)] flex-col">
      ...
      {/* Tabs skeleton */}
      <div className="flex gap-1 border-b bg-white px-6 py-1">
      ...
      <div className="flex-1 bg-gray-50 p-6 space-y-6">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="rounded-lg bg-white p-4 shadow-sm space-y-3">
            <div className="h-4 w-32 animate-pulse rounded bg-gray-200" />
            <div className="grid grid-cols-3 gap-4">
              {Array.from({ length: 3 }).map((_, j) => (
                <div key={j} className="h-9 animate-pulse rounded bg-gray-200" />
              ))}
            </div>
          </div>
        ))}
