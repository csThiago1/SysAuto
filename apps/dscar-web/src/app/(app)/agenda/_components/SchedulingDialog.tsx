      ...
      <DialogContent className="flex max-h-[90dvh] flex-col sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col mt-2">
          ...
          <div>
            ...
            {selectedOsLabel ? (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-foreground/90 flex-1">{selectedOsLabel}</span>
                <button
                  type="button"
                  ...
                  <div className="absolute z-10 mt-1 w-full rounded-md border border-border bg-muted/50 shadow-lg max-h-48 overflow-y-auto">
                    {osData.results.map((os) => (
                      ...
                      >
                        ...
                        <span className="ml-2 text-muted-foreground">{os.plate}</span>
                        {os.customer_name && <span className="ml-2 text-muted-foreground text-xs">{os.customer_name}</span>}
                      </button>
                    ))}
                  </div>
                )}
          ...
          <div>
            ...
            <input type="number" min="0" className={INPUT} placeholder="Ex: 5" {...register("repair_days")} aria-invalid={!!errors.repair_days} />
            {estimatedDelivery && (
