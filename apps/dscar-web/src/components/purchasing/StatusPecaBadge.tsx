...
const STATUSES: Record<string, { label: string; dotColor: string; textColor: string }> = {
  ...
  manual: { label: "Manual", dotColor: "bg-muted-foreground", textColor: "text-muted-foreground" },
}

export function StatusPecaBadge({ status, className = "" }: StatusPecaBadgeProps) {
  const config = STATUSES[status] ?? { label: status, dotColor: "bg-muted-foreground", textColor: "text-muted-foreground" }
  return (
