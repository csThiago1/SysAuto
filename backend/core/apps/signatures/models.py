...
class Signature(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["orcamento", "document_type", "-signed_at"],
                         name="sig_orcamento_doctype_idx"),
        ]
        constraints = [
            # Pelo menos um de service_order ou orcamento deve estar preenchido
            models.CheckConstraint(
                check=(
                    models.Q(service_order__isnull=False)
                    | models.Q(orcamento__isnull=False)
                ),
